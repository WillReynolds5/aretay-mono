...
type QuestionItem = { parentSegmentKey: string; questionIndex: number; label: string };
...
async function buildPlan(courseId: string): Promise<BulkPlan> {
  ...
  const questions: QuestionItem[] = flattenQuestions(curriculum)
    .filter(q => !cards.get(q.questionKey)?.video_r2_key)
    .map(q => ({ parentSegmentKey: q.parentSegmentKey, questionIndex: q.questionIndex, label: q.questionKey }));
  ...
  return { needsCover: !data.cover_image_url, segments, captionOnly, questions };
}
...
async function runJob(job: BulkJob, plan: BulkPlan, origin: string, videoRetries: number) {
  ...
  async function produceQuestion(q: QuestionItem) {
    let lastError: unknown = null;
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        await postJson(origin, "/api/generate-question-video", {
          courseId,
          segmentKey: q.parentSegmentKey,
    ...
    throw lastError ?? new Error("Question video failed");
  }
  ...
  // Segments first — their boards are reused by the question videos below.
  ...
  await pool(plan.questions, q => runItem(q.label, () => produceQuestion(q)));
}
