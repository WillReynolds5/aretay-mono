...
export type Curriculum = {
  ...
  /** Populated by the course detail API: per-question spaced-repetition assets, keyed by question key. */
  question_assets?: Record<string, QuestionAsset>;
};
...
export type QuestionAsset = {
  ...
  /** Video length in whole seconds — the question audio rounded up. */
...
//
//  Each flashcard question gets its own short video: the narrator
//  asks the question, the clip runs ceil(audio seconds). Assets
//  live on a cards row keyed `L{order}-S{n}-Q{i}`.
// ------------------------------------------------------------
