...
import type { Curriculum, QuestionAsset } from "@/lib/curriculum";
import { flattenQuestions, flattenScripts, sanitizeCurriculum } from "@/lib/curriculum";
import { getVideoUrl, objectExists, segmentVideoKey } from "@/lib/r2";
import { getSupabaseAdmin } from "@/lib/supabase-admin";

const COURSE_COLUMNS =
  "id, title, description, cover_image_url, visibility, is_live, tags, curriculum, created_at, deleted_at";

async function buildQuestionAssets(
  curriculum: Curriculum,
  cardsMap: Map<string, CardData>,
): Promise<Record<string, QuestionAsset>> {
  const assets: Record<string, QuestionAsset> = {};

  await Promise.all(
    flattenQuestions(curriculum).map(async q => {
      const card = cardsMap.get(q.questionKey);
      if (!card) return;
      const production = card.production;

      assets[q.questionKey] = {
        audio_url: production.audio_r2_key ? await getVideoUrl(production.audio_r2_key) : null,
        video_url: card.video_r2_key ? await getVideoUrl(card.video_r2_key) : null,
        alt_answers: production.alt_answers ?? null,
        audio_duration: production.audio_duration ?? null,
        video_duration: production.video_duration ?? null,
  ...
  try {
    ...
    if (curriculum?.lessons?.length) {
      try {
        ...
        const question_assets = await buildQuestionAssets(curriculum, cardsMap);
        data.curriculum = { ...curriculum, scripts, question_assets };
      } catch (enrichErr) {
